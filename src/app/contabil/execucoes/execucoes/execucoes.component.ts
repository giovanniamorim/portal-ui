...
export class ExecucoesComponent implements OnInit, AfterViewInit  {
  ...
  ngAfterViewInit() {
    this.listarExecucoes({ page: "0", size: "5" })
  ...
  public listarExecucoes = (request:any) => {
    ...
    this.execucaoService
        ...
        .subscribe(
            (execucao) => {
                this.datasource = new MatTableDataSource(execucao.content) ;
                this.datasource.sort = this.sort;
                this.carregando = false;
                this.totalElements = execucao.totalElements
            },
            (error) => {
...
        this.listarExecucoes(request);
    }


  onDelete(Execucao: IExecucao){

    Swal.fire({
      title: 'Deseja remover o Execução?',
      ...
      if (result.isConfirmed) {
        this.execucaoService.remove(Execucao).subscribe( res => {
          this.listarExecucoes({ page: "0", size: "5" })
        })
        Swal.fire(
          'Removido!',
          'O Execução foi removida com sucesso.',
          'success'
        )
      } 
    })
